...
# 训练命令示例： # python train.py --model alexnet --num_classes 10
############################################################################################################
import os 
import argparse 
import math
...
parser.add_argument('--data_path', type=str, default=r"./cifar10")
...
def main(args):
    ...
    # 使用 PyTorch 内置的 CIFAR-10 数据集
    train_dataset = datasets.CIFAR10(root=args.data_path, train=True, download=True, transform=data_transform["train"])
    val_dataset = datasets.CIFAR10(root=args.data_path, train=False, download=True, transform=data_transform["val"]) 
 
    if args.num_classes != 10:
        raise ValueError("CIFAR-10 dataset has 10 classes, but input {}".format(args.num_classes))
    ...
    for epoch in range(args.epochs):
        ...
        with open(os.path.join(save_path, "vision_transformer_cifar10_log.txt"), 'a') as f: 
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), os.path.join(save_path, "vision_transformer_cifar10.pth")) 

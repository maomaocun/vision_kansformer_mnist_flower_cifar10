...
# kansformer1     kansformer2    vision_transformer1     vision_transformer2      vision_transformer_small   vision_transformer_big   swin_transformer_tiny   swin_transformer_small   swin_transformer 
...
def main(args):
    ...
    # create model
    model = classic_models.find_model_using_name(opt.model, num_classes=opt.num_classes,has_logits=False).to(device) 
    ...
    for epoch in range(args.epochs):
        ...
        with open(os.path.join(save_path, "vision_transformer_flower_log.txt"), 'a') as f: 
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), os.path.join(save_path, "vision_transformer_flower.pth")) 
